...
#        print(node.__dir__())
        try:
            ...
            x = node2name(node)
            if 'ctx' in fields:
                print("CLASS: %r ctx %r" % (node.__class__, node.ctx.__class__))
                #        if len(x) == 1:
                #            print("NODES VISIT[%d] %s" % (self.level, x[0]))
                #        else:
                #            print("NODES VISIT[%d] %s - %s" % (self.level, x[0], x[1]))
        except Exception as err:
            print(repr(err), repr(node.__class__))
    ...
    def visit_ImportFrom(self, node):
        module = node.module
        if module == 'docutils':
            # our interest is in names
            for alias in node.names:
                if alias.name == 'nodes':
                    asname = alias.asname or alias.name
                    self.nodes_name.append(asname)
        print("NODES_NAME[%d]: %s" % (self.level, self.nodes_name))
        self.generic_visit(node)
...
docutils_dir = '/home/jade/docutils-dev/venv2/lib/python3.7/site-packages/docutils'
